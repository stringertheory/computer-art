...
def new_cube(xyz, dx=0):
    """ Creates a new PyODE cude at position (x,y,z) """
    x, y, z = xyz
    xyz = (x + jitter(dx), y + jitter(dx), z + jitter(dx))
    body = ode.Body(world)
    M = ode.Mass()
...
cubes = [new_cube(xyz, jitter_amount) for xyz in [
    (0.5,3,0.5),
    (0.5,4,0),
    (0,5,0),
    (-0.5,6,0),
...
filename = f'cubes-{jitter_amount}-{seed}.gif'

clip = VideoClip(make_frame, duration=duration)
clip.write_gif(filename, fps=20)  # lossless format
